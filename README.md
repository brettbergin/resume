...
CI runs the Vitest suite (`npm test`) as the first step of the `check` job,
before Lint / Type-check / Build. Running `npm test` locally first is still
a good habit for a faster feedback loop before opening a pull request.
